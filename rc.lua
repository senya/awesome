...
beautiful.wallpaper = "/home/vsementsov/.config/awesome/lotr_wallpaper.jpg"
...
globalkeys = gears.table.join(
	awful.key({ modkey }, "=", function()
		scratch.drop("alacritty", "top", "left", 0.5, 0.5, true)
	end),
	...
	awful.key({ modkey }, "u", function()
		...
		for s in screen do
			...
			for _, tag in ipairs(s.tags) do
				mystore[output][tag.name] = tag:clients()
			end
		end
	end, { description = "bebebe", group = "bebebe" }),
	...
	awful.key({ modkey }, "i", function()
		awful.util.spawn_with_shell("autorandr --load horizontal")
	end, { description = "bebebe", group = "bebebe" }),
...
tag.connect_signal("request::screen", function(t)
	...
	if not (fallback_tag == nil) then
		...
		if output == nil then
			naughty.notify({ text = "nil Removing" })
			return
		end
		naughty.notify({ text = output .. " Removing" })
